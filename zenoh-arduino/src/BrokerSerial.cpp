...
void BrokerSerial::init()
{
  ...
  { INFO("uC RXD %d ", bs.size()); };
  _frameToCbor >> [&](const cbor& cb ){
    INFO("uC RXD CBOR %s ", cbor::debug(cb).c_str()); 
  };
  ...
  incomingCbor >> [&](const cbor &in)
  {
    ...
    {
      connected = true;
      INFO(" subscribers : %d publishers : %d ", _subscribers.size(), _publishers.size());
